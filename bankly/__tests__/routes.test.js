...
describe("POST /auth/login", function () {
  ...
  /* test bug #3.  added await keyword to authenticate method in models/user.js */
  test("should login admins with admin true", async function () {
    const response = await request(app).post("/auth/login").send({
      username: "u3",
      password: "pwd3",
    });
...
describe("GET /users/[username]", function () {
  ...
  /* test bug #1.  added jwt.verify to authUser middleware */
  test("should return 401 if invalid token provided", async function () {
    const response = await request(app)
      .get("/users/u1")
      .send({ _token: "invalid" });
    expect(response.statusCode).toBe(401);
  ...
  /* test bug #2.  added error handling for user not found */
...
describe("PATCH /users/[username]", function () {
  ...
  /* test bug #4. replaced requireAdmin with requireCorrectUserOrAdmin */
  test("should allow non admin user to patch themselves", async function () {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new-fn" }); // u1 is not admin
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new-fn",
